    ...
    <div class="container py-5 px-4">
      <div class="apply-form-container">
        ...
        <form @submit.prevent="submitProposal">
          ...
          <div class="form-section"><h2 class="section-title">Proposal price</h2><input type="number" min="1" step="any" v-model="formData.price" placeholder="Within client's budget" class="form-input"></div>
          ...
          <div class="form-actions">
            <button type="submit" class="btn-submit" :disabled="submitting">{{ submitting ? 'Submitting…' : 'Submit a proposal' }}</button>
            <button type="button" @click="cancel" class="btn-cancel" :disabled="submitting">Cancel</button>
          </div>
        </form>
      </div>
    </div>
...
export default {
  ...
  async mounted() {
    // Optionally pull the real project so the header shows live data; fall back to
    // the mock above so the page never breaks if the request fails.
    try {
      const { data } = await userAPI.getJobDetails(this.$route.params.id)
      const job = data?.data
      if (job) {
        this.project = {
          clientName: job.client?.name || this.project.clientName,
          title: job.title || this.project.title,
          description: job.description || this.project.description,
          budget: job.budget != null ? `$${job.budget}` : this.project.budget,
          deadline: job.time_to_finish || this.project.deadline,
          category: job.speciality || this.project.category
        }
      }
    } catch (err) {
      // Keep the mock project on failure.
    }
  },
  methods: {
    // ✅ Chatbot methods
    ...
    async submitProposal() {
      if (!(this.formData.agreeRequirements && this.formData.agreeBinding && this.formData.agreeTerms)) {
        this.toast.error('Please agree to all terms and conditions.')
        return
      }
      const budget = parseFloat(this.formData.price)
      if (!(budget > 0)) {
        this.toast.error('Please enter a valid proposal price.')
        return
      }
      this.submitting = true
      try {
        await userAPI.applyForJob(this.$route.params.id, {
          description: this.formData.proposal,
          budget,
          time_to_finish: this.formData.deliveryTime
        })
        this.toast.success('Your proposal has been submitted successfully!')
...
.btn-submit:disabled, .btn-cancel:disabled { opacity: 0.6; cursor: not-allowed; transform: none; }
