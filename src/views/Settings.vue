...
export default {
  ...
  data() {
    return {
      ...
      privacySettings: [
        { title: 'Show profile to other students', description: 'Make profile visible and searchable by other students', type: 'image', src: '/assets/search account.png', enabled: true },
  ...
  async mounted() {
    // Prefill the account tab from the logged-in user.
    const user = this.auth.state.user || {}
    this.accountData.email = user.email || ''
    this.accountData.username = user.username || ''
    this.accountData.accountType = user.type === 'client' ? 'Client' : 'Student'
    this.profileData.fullName = [user.first_name, user.last_name].filter(Boolean).join(' ')

    // Prefill the profile tab from the saved profile.
    try {
      const { data } = await userAPI.getMyProfile()
      const p = data?.data
      if (p) {
        this.profileData.speciality = p.speciality || ''
        this.profileData.bio = p.brief || ''
  ...
  methods: {
    ...
    saveAccount() {
      // The backend has no endpoint to change email/username yet.
      this.toast.info('Account details can\'t be changed yet.')
    ...
    async saveProfile() {
      ...
      try {
        ...
        const payload = {
          ...
          speciality: this.profileData.speciality || null,
          brief: this.profileData.bio || null,
          university: this.profileData.university || null,
          college: this.profileData.major || null,
