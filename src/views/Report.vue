  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        ...
        // The backend stores reports as JSON. A selected file is noted by name in
        // the description (binary upload for reports is not wired yet — see
        // docs/DEMO_GAPS.md). `attachment_url` can hold a link if you have one.
        const description = this.formData.file
          ? `${this.formData.description}\n\n(Attached file: ${this.formData.file.name})`
          : this.formData.description
        ...
        this.error = err.response?.data?.message || 'Could not submit your report. Please try again.'
      } finally {
