...
export default {
  ...
  methods: {
    async loadSavedJobs() {
      this.loading = true
      try {
        const { data } = await clientAPI.getClientSavedJobs()
        this.jobs = data?.data || []
      } catch (err) {
        // Backend unreachable or not a client — show an empty list rather than crash.
        this.jobs = []
      } finally {
        this.loading = false
      }
    ...
    async unsave(job) {
      ...
      try {
        await clientAPI.unsaveJob(job.id)
...
<style scoped>
.client-saved-page { min-height: 100vh; background-color: #F5F5F5; padding-bottom: 50px; }
