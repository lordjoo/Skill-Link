...
export default {
  ...
  data() {
    return {
      ...
      privacySettings: [
        { title: 'Show profile to freelancers', description: 'Make profile visible and searchable by freelancers', type: 'image', src: '/assets/search account.png', enabled: true },
  ...
  async mounted() {
    // Prefill the account tab from the logged-in user (email/username are readonly).
    const user = this.auth.state.user || {}
    this.accountData.email = user.email || ''
    this.accountData.username = user.username || ''
    this.accountData.accountType = 'Client'
    this.profileData.fullName = [user.first_name, user.last_name].filter(Boolean).join(' ')

    // Prefill the profile/company tab from the saved client profile.
    try {
      const { data } = await clientAPI.getClientProfile()
      const c = data?.data
      if (c) {
        this.accountData.email = c.email || this.accountData.email
        this.accountData.username = c.username || this.accountData.username
        ...
        this.profileData.location = c.location || ''
        // Keep languages around so a profile save doesn't wipe them (no UI field for them here).
        this.languages = Array.isArray(c.languages) ? c.languages : []
      }
    } catch (err) {
  ...
  methods: {
    ...
    async saveProfile() {
      ...
      try {
        const [firstName, ...rest] = this.profileData.fullName.trim().split(/\s+/)
        const payload = {
          first_name: firstName || null,
          last_name: rest.join(' ') || null,
          location: this.profileData.location || null,
          languages: this.languages,
          about: this.profileData.bio || null,
          company_name: this.profileData.companyName || null,
          industry: this.profileData.industry || null,
          website: this.profileData.website || ''
        }
        const { data } = await clientAPI.updateClientProfile(payload)
        // Keep the cached user name in sync so the navbar/profile update too.
        const c = data?.data
